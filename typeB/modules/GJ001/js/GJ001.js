...
function initializeGJ001() {
	...
	//绑定动作
	//输入框清除按钮点击
	mui('.mui-content').on('click', '.mui-icon-close-filled', function() {
		exchangeOneInput.value = '';
		exchangeTwoInput.value = '';
	});
	//货币对选择
	mui('.mui-content').on('change', '.money-type-select', function() {
		//选择后设置代表国旗与货币名称
	...
	//银行选择
	mui('.mui-content').on('change', '.hidden-select', function() {
		//银行名变更
