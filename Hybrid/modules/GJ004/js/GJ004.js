...
function initializeGJ004() {
	...
	mui('.input-layer').on('change', '.hidden-select', function() {
		...
		if (this.id.substring(0, 4) == 'from') {
			...
			document.getElementById('fromType').innerHTML = this.options[this.selectedIndex].text + '<span class="mui-icon iconfont icon-moreSelect" style="font-size: 14px;"></span>';
		} else {
			initToType = this.value;
			document.getElementById('toType').innerHTML = this.options[this.selectedIndex].text + '<span class="mui-icon iconfont icon-moreSelect" style="font-size: 14px;"></span>';
		}
		//切换后设置焦点
		textInput.focus();
		//初始化
