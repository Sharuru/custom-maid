	...
	mui.ajax(serverAddr + 'tools/express', {
		...
		success: function(requestData) {
			if (requestData.status != '200') {
				...
				//基础信息：快递公司和快递单号
				var baseInfoStr = '';
				baseInfoStr += '<div class="base-information"><p class="font-w300-s16">' + expressName + ' : ' + trackingNum + '<\p></div>';
				//快递详细信息
				var liStr = '';
				//判断最新一条快递信息
				...
				liStr += '<p class="font-w300-s16">' + requestData.data[0].time + '</p>';
				liStr += '<p class="font-w500-s16">' + requestData.data[0].context + '</p>';
				liStr += '</div></li>';
				for (var i = 1; i < requestData.data.length; i++) {
					liStr += '<li><i class="mui-icon iconfont icon-expressInfo list-timeline-icon"></i>';
					liStr += '<div class="list-timeline-content">';
					liStr += '<p class="font-w300-s16">' + requestData.data[i].time + '</p>';
					liStr += '<p class="font-w500-s16">' + requestData.data[i].context + '</p>';
					liStr += '</div></li>';
				}
...
function getHistory() {
	...
	if (hisSearch == null) {
		hisLayer.innerHTML = '<p class="font-w300-s16">暂无历史查询记录...</p>';
	} else {
		hisLayer.innerHTML = '<p class="font-w300-s16">历史查询记录</p>';
		...
		if (hisList.length % 2 == 0) {
			...
			for (var i = 0; i < hisList.length / 2; i++) {
				hisContext += '<tr><td class="history-record font-w300-s14">' + hisList[2 * i].name + ': ' + hisList[2 * i].num;
				hisContext += '<input type="hidden" value="' + hisList[2 * i].code + '" /></td>';
				hisContext += '<td class="history-record font-w300-s14">' + hisList[2 * i + 1].name + ': ' + hisList[2 * i + 1].num;
				hisContext += '<input type="hidden" value="' + hisList[2 * i].code + '" /></td>';
				hisContext += '</tr>';
			}
		} else {
			...
			for (var i = 0; i < parseInt(hisList.length / 2 + 1); i++) {
				if (i == parseInt(hisList.length / 2)) {
					hisContext += '<tr><td class="history-record font-w300-s14">' + hisList[2 * i].name + ': ' + hisList[2 * i].num;
					hisContext += '<input type="hidden" value="' + hisList[2 * i].code + '" /></td>';
					hisContext += '<td class="history-record font-w300-s14"> </td></tr>';
				} else {
					hisContext += '<tr><td class="history-record font-w300-s14">' + hisList[2 * i].name + ': ' + hisList[2 * i].num;
					hisContext += '<input type="hidden" value="' + hisList[2 * i].code + '" /></td>';
					hisContext += '<td class="history-record font-w300-s14">' + hisList[2 * i + 1].name + ': ' + hisList[2 * i + 1].num;
